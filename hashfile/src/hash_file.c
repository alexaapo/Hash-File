...
HT_ErrorCode HT_OpenIndex(const char *fileName, int *indexDesc){
  ...
  *indexDesc=q;  //Return the position of this hash file into the global array to variable indexeDesc. 
  ...
  if(integer!=200)  return HT_ERROR;  //200 is the special number for hash_files.
...
HT_ErrorCode HT_InsertEntry(int indexDesc, Record record) {
  ...
  else  //We have already visited this bucket at least once. 
  {
    ...
    {
      //Store the number of next_block to previous block, before the allocation.
      int next_block, count_record=1;
      CALL_BF(BF_GetBlockCounter(file_desc,&next_block));
      memcpy(data+sizeof(int),&next_block,sizeof(int));
      BF_Block_SetDirty(block);
      CALL_BF(BF_UnpinBlock(block));
      //Allocate a new block and write into it its first record.
      CALL_BF(BF_AllocateBlock(file_desc,block));
      data=BF_Block_GetData(block);
      memcpy(data,&count_record,sizeof(int));
...
HT_ErrorCode HT_PrintAllEntries(int indexDesc, int *id) {
  ...
  if(id!=NULL)
  {
    ...
    memcpy(&temp,data+hash*sizeof(int),sizeof(int));  //Take the key of the bucket.
    CALL_BF(BF_UnpinBlock(block));
    //If bucket has no blocks at all.
    if(temp==0)
    {
    ...
    while(flag==0)
    {
      ...
      //If we haven't found the record yet.
      if(flag==0)
      {
        memcpy(&temp,data+sizeof(int),sizeof(int));
        //Case: We are in the last block of chain.
        if(temp==0)
        {
          printf("There isn't any record with record id: %d in this Hash File.\n",*id);
          return HT_OK;
  ...
  else
  {
    ...
    for(int j=1;j<=num_of_blocks;j++)
    {
      ...
      { 
        ...
        //Get all blocks of chain successively and print all the records of the hash file.
        while(temp!=0)
        {          
          printf("BLOCK %d \n", temp);
          CALL_BF(BF_GetBlock(file_desc,temp,block));
    ...
    //Case: All buckets have no records(an empty hash file).
    if(no_recs==buckets)
    {
...
HT_ErrorCode HT_DeleteEntry(int indexDesc, int id) {
  ...
  while(flag==0)
  {
    ...
    {
      memcpy(&rec,data+eight_bytes+i*sizeof(Record),sizeof(Record));
      //We just found the record that is to be deleted.
      if(rec.id==id)
      {
        ...
        {  
          ...
          //Write the last record to the position of record that is to be deleted.
